...
u64 g2va_to_g1pa(struct kvm_vcpu *vcpu, struct vmcs12* vmcs12, u64 addr){
	...
	/* Guest Level 1: VMM does not use EPT and has no virtual memory */
	if(!nested_cpu_has_ept(vmcs12)){
		gfn = vcpu->arch.mmu->gva_to_gpa(vcpu, addr, 0, NULL) >> 12;
	}

	/* Guest Level 1: VMM does use EPT and has no virtual memory */
	else {
		gfn = addr >> 12;
	}

	u64 real_gfn = (u64)(vcpu->arch.walk_mmu->translate_gpa(vcpu, ((u64)gfn) << 12, 0, NULL));
