...
static int vmx_pt_setup_topa(struct vcpu_vmx_pt *vmx_pt)
{
	...
	*  |Topa Entry_C (PTR, END)                |--------------------------/ (3. force trace stop, ptr to Entry_A)
	*  +---------------------------------------+
	*/
...
void vmx_pt_setup_vmx_autoload_msr(struct vcpu_vmx_pt *vmx_pt_config, bool enable_vmx_pt){
	...
	/* set IA32_RTIT_CTL_MSR.TraceEn for guest msr */
	guest_val = host_val;
	if(enable_vmx_pt)
		guest_val |= TRACE_EN;
...
static int vmx_pt_check_support(void){
	/* 
	 *  Let's assume that all logical cpu cores support the following features:
	 *  - Intel PT
	 *	- VMX supported tracing
	 *	- ToPA support for multiple reagions
