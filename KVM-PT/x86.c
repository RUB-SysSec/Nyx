...
#include "header/uapi/linux/kvm_para.h"
#include "header/linux/kvm_host.h"
#include "irq.h"
#include "mmu.h"
#include "i8254.h"
#include "tss.h"
#include "header/x86/kvm/kvm_cache_regs.h"
#include "x86.h"
#include "cpuid.h"
#include "pmu.h"
#include "hyperv.h"
...
#include "header/linux/kvm_irqfd.h"
...
extern int kvm_cpu_has_pending_timer2(struct kvm_vcpu *vcpu);
extern void kvm_check_async_pf_completion2(struct kvm_vcpu *vcpu);
	...
	if (nr == HYPERCALL_KAFL_RAX_ID){
        ...
        vcpu->run->hypercall.args[0] = a1;
			...
			case (KVM_EXIT_KAFL_USER_ABORT-KAFL_EXIT_OFFSET):
				...
				break;
			case (KVM_EXIT_KAFL_RANGE_SUBMIT-KAFL_EXIT_OFFSET):
				vcpu->run->exit_reason = KVM_EXIT_KAFL_RANGE_SUBMIT;
				vcpu->run->hypercall.args[0] = a1;
				break;
			case (KVM_EXIT_KAFL_REQ_STREAM_DATA-KAFL_EXIT_OFFSET):
				vcpu->run->exit_reason = KVM_EXIT_KAFL_REQ_STREAM_DATA;
				vcpu->run->hypercall.args[0] = a1;
				break;
			case (KVM_EXIT_KAFL_PANIC_EXTENDED-KAFL_EXIT_OFFSET):
				vcpu->run->exit_reason = KVM_EXIT_KAFL_PANIC_EXTENDED;
				vcpu->run->hypercall.args[0] = a1;
				break;
			default:
				r = -KVM_EPERM;
...
static int vcpu_run(struct kvm_vcpu *vcpu)
{
	...
	for (;;) {
		...
		if (kvm_cpu_has_pending_timer2(vcpu))
		...
		kvm_check_async_pf_completion2(vcpu);
